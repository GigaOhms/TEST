...
{
    S1 = ((va[i] - 0.5) * m + 0.5) * TOP0 + 0.5;           // PWM Pin 6
    S2 = ((vb[i] - 0.5) * m + 0.5) * TOP0 + 0.5;           // PWM Pin 5
    S3 = ((vc[i] - 0.5) * m + 0.5) * TOP0 + 0.5;           // PWM Pin 11
...
#define ONE_by_VDC  0.01169590643
#define PU          0.000244140625
#define ONE_b_200	0.0050

volatile uint16_t i = 0, analogA5 = 0;
volatile uint16_t S1, S2, S3;
volatile uint8_t STT;
volatile double t = 0.0, m = 1.0, VREF = 1000.0, sumAnalog = 0.0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	m = VREF * PU * 3.3 * K1 * SQRT3 * ONE_by_VDC;
    m = m > 0.95 ? 0.95 : m;
    m = m < 0.05 ? 0.05 : m;
    m = 1.0;
