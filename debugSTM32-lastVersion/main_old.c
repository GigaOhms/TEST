...
volatile uint32_t frequenceCounter = 65535;
/* USER CODE END PV */
...
//	VREAL = VREAL <= VMIN ? 0 : VREAL;
    ...
    /* USER CODE BEGIN 3 */
	  if (frequenceCounter > 899){
		  frequenceCounter = frequenceCounter - 1;
		  TIM1->ARR = frequenceCounter;
//		  HAL_Delay(1);
	  } else TIM1->ARR = 899;
//	  htim1.Init.Period = frequenceCounter;
